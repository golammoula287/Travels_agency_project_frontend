...
//                           )}{" "}
//                           —{" "}
//                           {new Date(?.discountTimeFrameDate.startDate).toLocaleDateString(
//                             "en-GB",
//                             {
//                               day: "2-digit",
//                               month: "short",
...
//       setFinalPrice(defaultPrice);
//     }
//   }, [propertyData, selectedPackage, searchValues]);
...
//                         ) : (
//                           <>
//                             {Number(item.ConvertedPrice).toFixed(2)} USD)
//                           </>
//                         )}
//                       </span>
//                     </div>
...
//                         )}{" "}
//                         to{" "}
//                         {dayjs(propertyData?.discountTimeFrame?.endDate).format(
//                           "DD MMM, YYYY"
//                         )}
//                       </p>
//                     )}
...
function ResortAndPrice({ propertyData }) {
  ...
  useEffect(() => {
    const searchStartMonth = dayjs(searchValues?.tripStart).month();
    const searchStartYear = dayjs(searchValues?.tripStart).year();

    const searchEndMonth = dayjs(searchValues?.tripEnd).month();
    const searchEndYear = dayjs(searchValues?.tripEnd).year();

    const discountStartMonth = dayjs(propertyData?.discountTimeFrame?.startDate).month();
    const discountStartYear = dayjs(propertyData?.discountTimeFrame?.startDate).year();

    const discountEndMonth = dayjs(propertyData?.discountTimeFrame?.endDate).month();
    const discountEndYear = dayjs(propertyData?.discountTimeFrame?.endDate).year();

    const discountPercentage = Number(propertyData?.discount);
    const defaultPrice = Number(selectedPackage?.ConvertedPrice);

    // Check if search month and year overlap with the discount time frame
    const isDiscountActive =
      (searchStartYear === discountStartYear && searchStartMonth === discountStartMonth) ||
      (searchStartYear === discountEndYear && searchStartMonth === discountEndMonth) ||
      (searchEndYear === discountStartYear && searchEndMonth === discountStartMonth) ||
      (searchEndYear === discountEndYear && searchEndMonth === discountEndMonth);
